...
func emit_token(c *gosocketio.Channel) {
	mapboxToken := os.Getenv("MAPBOX_ACCESS_TOKEN")
	log.Printf("mapbox-token try to transfer [%s] ", mapboxToken)
	if *mapbox != "" {
		mapboxToken = *mapbox
	}
...
func run_server() *gosocketio.Server {
	...
	server.On("mapboxtoken", func(c *gosocketio.Channel) {
		// sending mapbox token from provider to browser.
		go emit_token(c)
	})
...
func main() {
	log.Printf("HarmovisProvider(%s) built %s sha1 %s", sxutil.GitVer, sxutil.BuildTime, sxutil.Sha1Ver)
	flag.Parse()
